...
# loop across data types of interest
for (data.type in c('TN','NPOC','moisture')) {

# removing everything except data.type just to be safe during the looping
all.objects = ls()
all.objects = all.objects[-which(all.objects == "data.type")]
rm(list=all.objects)
...
if (data.type == 'moisture') { # defines the data type specific set of file names and other inputs. customize for each data type.
  ...
  method.id.file = "ESS-DIVE_Chemistry_Template.xlsx" #file name for input, methods file with typical code numeric indicator

}

if (data.type == 'NPOC') { # defines the data type specific set of file names and other inputs. customize for each data type.
  
  data.file.name = "Corrected_Injection_NPOC-TN_ECA2.csv" # name of the file that has the data
  map.file.name = data.file.name # name of the file used for mapping, including the .csv at the end
  
  template.input.name = "Boye_Template_Input_NPOC.csv" # names of the file with the column names that are going to be in the data file and those desired to be put into the template
  template.file.name = "ECA2_NPOC_Report_Format_Filled.csv" # name of the output template file
  method.name = "NPOC_Typical" # the text string within the Method_Name from the method.id file
  sample_name_col_name = "Sample_ID" # the name of the column that is for the sampleNames
  digits.for.rounding = 2 # indicate the number of digits to round the data to
...
if (data.type == 'TN') { # defines the data type specific set of file names and other inputs. customize for each data type.
  
  data.file.name = "Corrected_Injection_NPOC-TN_ECA2.csv" # name of the file that has the data
  map.file.name = data.file.name # name of the file used for mapping, including the .csv at the end
  
  template.input.name = "Boye_Template_Input_TN.csv" # names of the file with the column names that are going to be in the data file and those desired to be put into the template
  template.file.name = "ECA2_TN_Report_Format_Filled.csv" # name of the output template file
  method.name = "TN_Typical" # the text string within the Method_Name from the method.id file
  sample_name_col_name = "Sample_ID" # the name of the column that is for the sampleNames
  digits.for.rounding = 2 # indicate the number of digits to round the data to
...
# potentially unique to the ECA project. trims sample names to a specified number of characters
trim.sample.name = T
start.trim = 1
stop.trim = 12

# read in mapping file
map.file = read.csv(map.file.name,stringsAsFactors = F)
head(map.file)
str(map.file)

# read in data file
dat.file = read.csv(data.file.name,stringsAsFactors = F)
head(dat.file)
str(dat.file)
...
# read in the methods ids file
method.ids = data.frame(read_excel(method.id.file, sheet = 1))

# read in the methods file with typical code numeric indicator
method.devs = data.frame(read_excel(method.dev.file, sheet = 2))
...
write.csv(methods.out,paste0("Methods_req_",method.name,".csv"),row.names = F,quote = F) 
...
if (nrow(report.template) - (9 + length(additional_row.names) + length(sample.names)) != 0) { print("Error: Number of sampleNames doesn't match the template")} else {
  ...
  template.input = read.csv(template.input.name,stringsAsFactors = F)
